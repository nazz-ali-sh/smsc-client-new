...
const OnboardingBuildings = () => {
  ...
  useEffect(() => {
    if (onboardingData?.steps?.step_5?.building_height) {
      const matchingCard = buildingTypes?.find(card => card?.value === onboardingData.steps.step_5.building_height)
  ...
  const handleBuildingHeightSubmit = (buildingHeight: string) => {
    ...
    <div className='flex flex-col items-center pt-10 mb-20'>
      ...
      <div className='bg-white p-8 pt-10 w-full mt-6 pb-10'>
        ...
        <div className='grid sm:grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4 pb-10 mt-10'>
          {buildingTypes?.map(({ title, value, image }) => (
            <div
              key={title}
              onClick={() => {
                setSelected(title)
                handleBuildingHeightSubmit(value)
              }}
