...
const PmaTrustpilotReviewsView = () => {
  const router = useRouter()
  const [selectedMethod, setSelectedMethod] = useState<'manually' | 'auto_sync' | null>(null)
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      ...
      } else if (selectedMethod === 'auto_sync') {
    ...
    onError: (error: any) => {
      const errorMessage = error?.response?.data?.message || 'Failed to save Trustpilot reviews preference.'
  ...
  const createPayload = (trustpilotReviews: string): PmaGoogleReviewsSetupPayload => {
  ...
  const handleReviewMethod = (method: 'manually' | 'auto_sync') => {
  ...
  React.useEffect(() => {
    const choice = onboardingData?.data?.step_5?.trustpilot_reviews

    if (choice === 'manual') setSelectedMethod('manually')
    else if (choice === 'auto') setSelectedMethod('auto_sync')
  }, [onboardingData])
  ...
  return (
    <div className='flex flex-col items-center pt-10 mb-20'>
      ...
      <div className='bg-white p-8 pt-10 w-full mt-6'>
        ...
        <Typography variant='body2' sx={{ fontSize: '14px', fontWeight: 400, color: '#696969', marginBottom: '24px' }}>
          How would you like to give Trustpilot reviews for your business? Please choose an option below.
        </Typography>
...
export default PmaTrustpilotReviewsView
