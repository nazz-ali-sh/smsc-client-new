...
const UserProfile = ({ userData }: UserProfileProps) => {
  const reviewsData = {
    google: {
      rating: parseFloat(userData?.data?.ratings_and_reviews?.google_rating || '4.2'),
      count: userData?.data?.ratings_and_reviews?.google_review_count || 18
    },
    trustpilot: {
      rating: parseFloat(userData?.data?.ratings_and_reviews?.trustpilot_rating || '4.2'),
      count: userData?.data?.ratings_and_reviews?.trustpilot_review_count || 18
    }
  }

  const primaryContactInfo = {
    name: userData?.data?.pma_user?.name || 'David Jon',
    email: userData?.data?.pma_user?.email || 'Debra.holt@example.com',
    website: userData?.data?.company_details?.website || 'Debraholt.com',
    primaryPhone: userData?.data?.pma_user?.mobile_number || '(219) 555-0114',
    landline: userData?.data?.company_details?.landline || '(219) 555-0114',
    address: userData?.data?.company_details?.address || '417 Washington Ave. Manchester, Kentucky 39495'
  }
